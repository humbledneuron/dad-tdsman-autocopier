...
def get_last_day_of_month(month_name, year=2025):
    ...
    try:
        ...
        # Return as datetime object for proper date formatting
        return datetime(year, month_number, last_day)
    except KeyError:
        print(f"Warning: Unknown month '{month_name}', using default date")
        return datetime(2025, 3, 31)  # Default to March 31, 2025
...
def transfer_to_excel(csv_files, excel_file, log_widget):
    ...
    try:
        ...
        # Create Calibri font, size 11
        font = xlwt.Font()
        font.name = 'Calibri'
        font.height = 220  # 11 point = 220 twips

        # Regular style with Calibri font
        style_regular = xlwt.XFStyle()
        style_regular.font = font
        
        # Right-aligned style for Challan Serial Reference
        ...
        # Date style for Payment/Credit Date
        style_date = xlwt.XFStyle()
        style_date.font = font
        style_date.num_format_str = 'DD/MM/YYYY'  # Date format as dd/mm/yyyy
        ...
        for file_idx, csv_file in enumerate(valid_csv_files):
            ...
            for _, csv_row in csv_df.iterrows():
                # Get the last day date as a datetime object
                ...
                # Use the file index as Challan Serial Reference - RIGHT ALIGNED
...
def create_gui():
    ...
    # Formatting info frame
    format_frame = ttk.LabelFrame(root, text="Formatting Information")
    format_frame.pack(padx=10, pady=5, fill="x")
    
    format_info = ttk.Label(format_frame, 
                           text="• All text will be formatted in Calibri, size 11\n"
                                "• Payment/Credit Date will be formatted as date (DD/MM/YYYY)\n"
                                "• Challan Serial Reference will be right-aligned",
                           justify="left")
    format_info.pack(padx=5, pady=5, fill="x", anchor="w")
